...
static int map3d_map(t_data *all)
{
	...
	while (j < all->height)
	{
		...
		if (!line_coords)
			ft_exit(all, ERR_MALLOC);
		ft_memset(line_coords, 0, all->width);
		i = 0;
		while (all->raw_map[j][i] != '\0' && i < all->width)
		{
			line_coords[i].x = i;
			line_coords[i].y = j;
			if (all->raw_map[j][i] == '1')
				...
				line_coords[i].z = 0;
			if (line_coords[i].z == 1)
				line_coords[i].colour = RED;
			else
				line_coords[i].colour = GREEN;
			i++;
		}
...
static int map_2d_map(t_data *all)
{
	...
	while (j < all->height)
	{
		...
		while (i < all->width)
		{
			...
			line_coords[i].colour = all->map3d[j][i].colour;
			i++;
		}
